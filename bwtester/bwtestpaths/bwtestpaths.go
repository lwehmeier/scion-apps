...
const (
	DefaultBwtestParameters               = "3,1000,30000,80000kbps"
	DefaultDuration                       = 3
	DefaultPktSize                        = 1000
	DefaultPktCount                       = 30000
	DefaultBW                             = 80000000
	WildcardChar                          = "?"
...
func runForAll(local snet.Addr, remote snet.Addr) []*spathmeta.AppPath{
...
//		receiveDone sync.Mutex // used to signal when the HandleDCConnReceive goroutine has completed
	...
	Check(err)
	var targetPaths []*spathmeta.AppPath
	targetPaths = runForAll(*clientCCAddr, *serverCCAddr)
...
//	}
	...
	Check(err)
	networkSpeedMap := make(map[*sciond.PathReplyEntry]([]int64))
	for _, currentPath :=range targetPaths {
		fmt.Printf("Testing path %s\n", currentPath.Entry.Path.String())
		down, up := runTest(serverDCAddr, clientDCAddr, currentPath.Entry, CCConn, clientBwpStr, serverBwpStr, clientPort, serverPort, flagset)
		networkSpeedMap[currentPath.Entry] = append(networkSpeedMap[currentPath.Entry], down)
		networkSpeedMap[currentPath.Entry] = append(networkSpeedMap[currentPath.Entry], up)
	}
	f, err := os.OpenFile("networkSpeedMap.scd", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
	if err != nil {
		panic(err)
	}
	defer f.Close()

	for k, v := range networkSpeedMap {
		down, up := v[0], v[1]
		var fpm *FwdPathMeta
		fpm = k.Path
		pathStr := fpm.String()
		outStr := fmt.Sprintf("%s downspeed: %d upspeed: %d\n", pathStr, down, up)
		f.WriteString(outStr)
		fmt.Printf(outStr)
	}
}
func runTest(serverDCAddr *snet.Addr, clientDCAddr *snet.Addr, pathEntry *sciond.PathReplyEntry, CCConn *snet.Conn,clientBwpStr string, serverBwpStr string, clientPort uint16, serverPort uint16, flagset map[string]bool) (int64,int64) {
	var (
		DCConn *snet.Conn
		clientBwp    BwtestParameters
		serverBwp    BwtestParameters
...
//				sres.IPAmin/1e6, sres.IPAmax/1e6)
			//TODO: remove return, implement error handling for each iteration
